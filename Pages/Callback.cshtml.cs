...
namespace APSSAMCognito.Web.Pages
{
    public class CallbackModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet(string code)
        {
            ...
            _logger.LogInformation($"response : {response}");  //change kiya hai --abhay
            if (response.IsSuccessful)
            {
                var body = JsonSerializer.Deserialize<RedirectResult>(response.Content);
                var redirectUrl = body.Url;
                _logger.LogInformation($"Abhay-body : {body}"); 
                _logger.LogInformation($"response : {redirectUrl}"); 
                _logger.LogInformation($"redirectUrl : {redirectUrl}");  //change kiya hai --abhay
        ...
        private async Task<CognitoToken> GetCognitoTokens(string code)
        {
            ...
            _logger.LogInformation($"uriBuilder.Query : {query.ToString()}"); 
            ...
            _logger.LogInformation($"Token Url : {uriBuilder.Uri}");
            var restClient = new RestClient();
            _logger.LogInformation($"restClient : {restClient}");
            var restRequest = new RestRequest(uriBuilder.Uri);
            _logger.LogInformation($"restRequest : {restRequest}");
            restRequest.AddHeader("content-type", "application/x-www-form-urlencoded");
            restRequest.AddHeader("Authorization", "Basic " + authHeader);
            _logger.LogInformation($"restRequest.addheader : {restRequest}");
            var cognitoToken = await restClient.PostAsync<CognitoToken>(restRequest);
            _logger.LogInformation($"cognitoToken : {cognitoToken}");  //change kiya hai --abhay
            return cognitoToken;
        }
        ...
        private  async Task<IRestResponse>  GetRedirectUrl(string idToken)
        {
            ...
            _logger.LogInformation($"Url Parameters :{queryParams}");  //change kiya hai --abhay
            ...
            _logger.LogInformation($"environment 129: {env}");
            ...
            _logger.LogInformation($"response 139 : {response}");
            return null;
            /*
            ...
            */
            ...
            _logger.LogInformation($"response 157 : {response}");
            return response;
